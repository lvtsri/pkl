...
if (isset($_POST['btn_impor'])){    //triger setelah btn_impor di klik
    ...
    for ($i=2; $i <= count($data_excel) ; $i++){    //perulangan
        ...
        if ($nik == '' || $nama == '' || $kontak == '' || $email == '' || $kelamin == '') {
            continue;
        }
        
        $query_cek = mysqli_query($koneksi, "SELECT nik FROM tb_dosen WHERE nik='$nik'")or die(mysqli_error($koneksi));

        if (mysqli_num_rows($query_cek)==0) {   //ketika tidak ada data kode matkul
            $query_insert = mysqli_query($koneksi, "INSERT INTO tb_dosen VALUES ('$nik', '$nama', '$kontak', '$email', '$kelamin')")or die(mysqli_error($koneksi));
    ...
    echo '<script>
    alert("Data dosen berhasil di impor");
