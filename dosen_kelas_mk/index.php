<?php
    ...
    echo '<script>
        ...
        alert("Anda bukan dosen!!! Akan segera di logout-kan");
    </script>';
    } else {

    $nik_dosen = $_SESSION['nik'];
    $panggil_data_dosen = mysqli_query($koneksi, "SELECT * FROM tb_dosen WHERE nama = '$nik_dosen'")or die(mysqli_error($koneksi));

    $data_utama_dosen = mysqli_fetch_assoc($panggil_data_dosen);
                    ...
                    <div class="card-body">
                      <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                          ...
                          if (!empty($filter)) {
                              $panggil_kelas = mysqli_query($koneksi, "SELECT * FROM tb_kelas_makul WHERE kode_akd = '$filter' AND nik = '$nik_dosen'") or die(mysqli_error($koneksi));
                          } else {
                              $panggil_kelas = mysqli_query($koneksi, "SELECT * FROM tb_kelas_makul WHERE nik = '$nik_dosen'") or die(mysqli_error($koneksi));
                          }
